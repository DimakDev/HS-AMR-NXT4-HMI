...
float poly0[] = {  4.9537, -48.437, 190.12, -385.62, 434.38, -272.12, 89.249};
//float poly0[] = {  456.35432065,-2420.14692571,5188.3459268,-5759.52569214,3518.77781422,-1160.15105363,188.58555355};
//Frontsidesensor  (an A1-FRONTSIDE)
//float poly1[] = {   4531.05056378,-20244.93592164,36543.23322573,-34071.96736949,17335.60829245,-4617.26573071,536.94761891};
float poly1[] = { 1416.4154322, -6034.60305331, 10379.05448915, -9263.49421469, 4592.17987269, -1256.40583596, 178.80377704};
//Backsensor       (an A2-BACK)
float poly2[] = {   2767.11924063, -11718.58881245, 19947.75068658, -17465.17069544, 8342.9481465, -2119.08563891, 256.51450215};
//Backsidesensor   (an A3-BACKSIDE)
float poly3[] = { 2876.21551915, -12251.4120095, 20955.40248927, -18417.04603586, 8823.19226667, -2244.7007624, 270.37043255};
//float poly3[] = {   12.82121843,-115.50337303,416.43290671,-769.88495393,777.40270763,-422.7665695,113.52481948};

//the Distancesensor-pins on the Arduino (Analogpin-)
const byte FRONT = 3;
const byte FRONTSIDE = 1;
const byte BACK = 0;
const byte BACKSIDE = 2;

//MausodometrieVariablen
//Mauskallibrierwerte
//const int Msx = 39;      //Pixel per mm
//const int Msy = 37;
const float Xsm = 0.024;
const float Ysm = 0.026;
...
void loop() {
  ...
  if (remoteSerial.available()) {
    remoteSerial.readBytes(status, 2);

    Serial.println("#################################");
    Serial.print((String)status[0] + (String)status[1]);
    Serial.println();
  }
  //HMI end
  ...
  if (serAvail()) {
    ...
    Serial.println("\n#########NXT sends!\nArray size:" + (String)innbuf);
    for (int i = 0; i < innbuf; i++) 
      Serial.print((String)(int)inBuf[i] + " ");
    ...
    if (innbuf > 1)
    {
      remoteSerial.write(inBuf + 1, innbuf - 1);
    }
    //HMI end
    ...
    if (inBuf[0] == 23) //Anfrage nach Sensordaten
    {
      ...
      */
      ...
      serSendMsg(outBuf, 16); // 14 + 2 bytes for HMI
      // measureComms(1);
    }
